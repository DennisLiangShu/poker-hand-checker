...
public class Hand implements Comparable<Hand> {
   ...
   public boolean isFourOfaKind() {
      if (rankFrequency.containsValue(4)) {
         return true;
      }
      return false;
   }
   ...
   public boolean isFullHouse() {
      if (rankFrequency.containsValue(3) && rankFrequency.containsValue(2)) {
         return true;
      }
      return false;
   }
   ...
   public boolean isThreeOfaKind() {
      if (rankFrequency.containsValue(3) && !isFullHouse()) {
         return true;
      }
      return false;
   }
   ...
   public boolean isTwoPair() {
      int count = 0;
      Collection<Integer> a = rankFrequency.values();
      for (int i : a) {
         if (i == 2) {
            count++;
         }
   ...
   public boolean isPair() {
      if (rankFrequency.containsValue(2) && !isTwoPair() && !isFullHouse()) {
         return true;
      }
      return false;
   }
   ...
   public int compareTo(Hand h) {
      ...
      switch (handValue) {
         ...
         case 8:
            for (int i = 0; i < hand.size(); i++) {
               if (cardRanks[i] > h.cardRanks[i] ) {
                  return 1;
               } else if (cardRanks[i] < h.cardRanks[i]) {
                  return -1;
               }
            }
            break;
         ...
         case 2:
            ...
            int sh = 0;
            for (int i = 1; i < hand.size(); i++) {
               if (cardRanks[i] == cardRanks[i-1] && cardRanks[i] > fh) {
                  fh = cardRanks[i];
               }
               if (h.cardRanks[i] == h.cardRanks[i-1] && h.cardRanks[i] > sh) {
                  sh = h.cardRanks[i];
               }
            }
            if (fh == sh) {
               for (int i = 0; i < hand.size(); i++) {
                  if (cardRanks[i] != h.cardRanks[i]) {
                     return cardRanks[i] - h.cardRanks[i];
                  }
               }
            } else {
               return fh - sh;
         ...
         case 6:
         case 7:
            return cardRanks[2] - h.cardRanks[2];
      }
      return 0;
   }
}
